...
class SimplePrior(nn.Module):
    def __init__(
        ...
        if self.x_cond:
            ...
            if dist.get_rank() == 0:
                print(f"Conditioning on 1 above level(s)")
        ...
        if labels:
            ...
            self.labeller = Labeller(
                self.y_emb.max_bow_genre_size, self.n_tokens, self.sample_length, v3=self.labels_v3
            )
        else:
    ...
    def prior_postprocess(self, z):
        ...
        for i in range(len(xs)):
            ...
            assert (
                xs[i] < bins
            ).all(), f"rank: {dist.get_rank()}, bins: {bins}, dims {dims}, shape {shape}, prior_shape {self.prior_shapes}, bins_shift {bins_shift}, xs[i]: {xs[i]}"
