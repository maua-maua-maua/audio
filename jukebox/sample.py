...
def sample_level(zs, labels, sampling_kwargs, level, prior, total_length, hop_length, hps):
    ...
    if total_length >= prior.n_ctx:
        ...
        for start in starts:
            ...
            print(
                f"Step "
                + colored(counterr, "blue")
                + "/"
                + colored(len(starts), "red")
                + " ~ New to Sample: "
                + str(newtosample)
                + " ~ estimated remaining minutes: "
                + (colored("???", "yellow"), colored(timex, "magenta"))[counterr > 1 and newtosample]
            )
    else:
...
def load_codes(codes_file, duration, priors, hps):
    ...
    if duration is not None:
        ...
        assert (
            duration % top_raw_to_tokens == 0
        ), f"Cut-off duration {duration} not an exact multiple of top_raw_to_tokens"
        assert (
            duration // top_raw_to_tokens <= zs[-1].shape[1]
        ), f"Cut-off tokens {duration//priors[-1].raw_to_tokens} longer than tokens {zs[-1].shape[1]} in saved codes"
...
def save_samples(model, device, hps, sample_hps):
    ...
    ), f"Upsampling needs atleast one ctx in get_z_conds. Please choose a longer sample length"
...
def run(model, mode="ancestral", codes_file=None, audio_file=None, prompt_length_in_seconds=None, port=29500, **kwargs):
    ...
    sample_hps = Hyperparams(
        dict(mode=mode, codes_file=codes_file, audio_file=audio_file, prompt_length_in_seconds=prompt_length_in_seconds)
    )
