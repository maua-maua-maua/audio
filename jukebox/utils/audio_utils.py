...
def calculate_bandwidth(dataset, hps, duration=600):
    ...
    while n_seen < n_samples:
        ...
        stft = librosa.core.stft(
            np.mean(samples, axis=1), hps.n_fft, hop_length=hps.hop_length, win_length=hps.window_size
        )
    ...
    mean = total / n_seen
    bandwidth = dict(l2=total_sq / n_seen - mean ** 2, l1=l1 / n_seen, spec=spec_norm_total / spec_nelem)
