...
class LossScalar(object):
    def __init__(self, loss_scale, init_scale=2.0 ** 16, scale_factor=2.0 ** (1.0 / 1000), scale_window=1):
        if loss_scale == None:
...
def grad_norm(params, scale, flat=False):
    ...
    if flat:
        ...
        fp16_norm = (
            0.0 if len(fp16_grads) == 0 else float(_flatten_dense_tensors(fp16_grads).norm(p=2, dtype=torch.float32))
        )
    ...
    def step(self, closure=None, scale=1.0):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if p.data.dtype == torch.float16:
                    state["scale_exp_avg"] = 1e-8 + float(torch.norm(exp_avg, float("inf"))) / self.FLOAT16_MAX
                    state["scale_exp_avg_sq"] = 1e-8 + float(torch.norm(exp_avg_sq, float("inf"))) / self.FLOAT16_MAX
