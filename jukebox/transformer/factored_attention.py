...
class FactoredAttention(nn.Module):
    ...
    def summary_spread_attn(self, q, k, v, sample):
        ...
        if sample:
            ...
            k = (
                t.nn.functional.pad(k.view(bs, blocks, l // blocks, d)[:, :-1, -spread:, :], (0, 0, 0, 0, 1, 0))
                .contiguous()
                .view(bs, blocks * spread, d)
            )  # bs, blocks * spread, d
            v = (
                t.nn.functional.pad(v.view(bs, blocks, l // blocks, d)[:, :-1, -spread:, :], (0, 0, 0, 0, 1, 0))
                .contiguous()
                .view(bs, blocks * spread, d)
            )  # bs, blocks * spread, d
            return self.dense_attn(q, k, v, sample).view(bs, l, d)
    ...
    def check(self):
        ...
        exp_pos_grad = {
            ...
            5: ((t.arange(pos) % (l // blocks) >= (l // blocks - spread)) & (t.arange(pos) < block_pos))
            .nonzero()
            .view(-1),
        }[self.attn_func]
    ...
    def check_sample(self):
        ...
        assert (
            max_err < 1e-8
        ), f"Max sampling err is {max_err} {[i for i in range(l) if t.max(t.abs(x_out_sample - x_out_normal)[:,i,:]) > 1e-8]}"
        ...
        assert (
            max_err < 1e-8
        ), f"Max prime sampling err is {max_err} {[i for i in range(prime) if t.max(t.abs(x_out_sample - x_out_normal)[:,i,:]) > 1e-8]}"
    ...
    def check_chunks(self, chunk_size):
        ...
        with t.no_grad():
            ...
            assert (
                max_err <= 1e-6
            ), f"Max err is {max_err} {[i for i in range(l) if t.max(t.abs(y_forw - y_forw_sample)[:, i, :]) > 1e-6]}"
