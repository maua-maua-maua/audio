...
def save_checkpoint(logger, name, model, opt, metrics, hps):
    with t.no_grad():
        ...
        save_hps = {
            ...
            if k not in ["metadata_v2", "metadata_v3", "alignments", "lyric_processor", "midi_processor"]
        }
        t.save(
            {
                "hps": save_hps,
...
def save_outputs(model, device, hps):
    ...
    for level in range(len(priors)):
        ...
        y_in = (
            t.from_numpy(prior.labeller.get_y_from_ids(artist_id, genre_ids, lyric_tokens, total_length, offset))
            .view(1, -1)
            .cuda()
            .long()
        )
