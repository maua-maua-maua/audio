...
def repeat(x, n, dim):
    if dim == -1:
        dim = len(x.shape) - 1
    return (
        x.reshape(int(np.prod(x.shape[: dim + 1])), 1, int(np.prod(x.shape[dim + 1 :])))
        .repeat(1, n, 1)
        .reshape(*x.shape[:dim], n * x.shape[dim], *x.shape[dim + 1 :])
    )
...
class DummyPrior:
    ...
    def ancestral_sample(self, n_samples, z_conds=None, y=None):
        z = t.zeros((n_samples, self.n_ctx), dtype=t.long, device="cuda") + t.arange(
            0, self.n_ctx, dtype=t.long, device="cuda"
        ).view(1, self.n_ctx)
        ...
        if z_conds is not None:
            ...
            assert (
                z // 4 == repeat(z_cond, 4, 1)
            ).all(), f"z: {z}, z_cond: {z_cond}, diff: {(z // 4) - repeat(z_cond, 4, 1)}"
        return z
    ...
    def primed_sample(self, n_samples, z, z_conds=None, y=None):
        ...
        start = z[:, -1:] + 1
        z_rest = (
            t.arange(0, self.n_ctx - prime, dtype=t.long, device="cuda").view(1, self.n_ctx - prime) + start
        ).view(n_samples, self.n_ctx - prime)
...
def test_primed_sample(labels, priors, hps):
    ...
    for z, z_in in zip(zs, zs_in):
        ...
        assert (
            (
                z_rest
                - t.arange(0, total_length - prime_length + 1, dtype=t.long, device="cuda").view(
                    1, total_length - prime_length + 1
                )
            )
            == 0
        ).all()
...
def check_sample():
    ...
    y = (
        t.tensor([max_total_length, offset, sample_length, 10, 1, -1, -1, -1, -1], dtype=t.long, device="cuda")
        .view(1, 9)
        .repeat(n_samples, 1)
    )
