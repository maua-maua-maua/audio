...
from jukebox.make_models import make_prior, make_vqvae, restore_opt, save_checkpoint
from jukebox.utils.audio_utils import audio_postprocess, audio_preprocess
from jukebox.utils.dist_utils import allgather, allreduce, print_once
from jukebox.utils.ema import CPUEMA, EMA, FusedEMA
from jukebox.utils.fp16 import FP16FusedAdam, FusedAdam, LossScalar, backward, clipped_grad_scale
from jukebox.utils.logger import init_logging
from jukebox.utils.torch_utils import count_parameters, zero_grad
...
def get_ddp(model, hps):
    ...
    ddp = DistributedDataParallel(
        model, device_ids=[local_rank], output_device=local_rank, broadcast_buffers=False, bucket_cap_mb=hps.bucket
    )
    return ddp


def get_ema(model, hps):
    mu = hps.mu or (1.0 - (hps.bs * hps.ngpus / 8.0) / 1000)
    ema = None
...
def get_optimizer(model, hps):
    ...
    if hps.fp16:
        rank = dist.get_rank()
        local_rank = rank % 8
        scalar = LossScalar(hps.fp16_loss_scale, scale_factor=2 ** (1.0 / hps.fp16_scale_window))
        if local_rank == 0:
...
def train(model, orig_model, opt, shd, scalar, ema, logger, metrics, data_processor, hps):
    ...
    for i, x in logger.get_range(data_processor.train_loader):
        ...
        loss, scale, grad_norm, overflow_loss, overflow_grad = backward(
            loss=loss, params=list(model.parameters()), scalar=scalar, fp16=hps.fp16, logger=logger
        )
