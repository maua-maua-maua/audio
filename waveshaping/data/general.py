...
class GeneralDataset(torch.utils.data.Dataset):
    def __init__(self, path: str, split: str = "train", load_to_memory: bool = True):
        ...
        self.data_list = [
            f.replace("audio_", "") for f in os.listdir(os.path.join(self.split_path, "audio")) if f[-4:] == ".npy"
        ]
        if load_to_memory:
            self.audio = [np.load(os.path.join(self.split_path, "audio", "audio_%s" % name)) for name in self.data_list]
            self.control = [
                np.load(os.path.join(self.split_path, "control", "control_%s" % name)) for name in self.data_list
            ]
...
class GeneralDataModule(pl.LightningDataModule):
    def __init__(self, data_root: str, batch_size: int = 16, load_to_memory: bool = True, **dataloader_args):
        super().__init__()
    ...
    def _make_dataloader(self, dataset):
        return torch.utils.data.DataLoader(dataset, self.batch_size, **self.dataloader_args)
