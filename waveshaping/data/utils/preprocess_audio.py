...
def segment_signal(
    ...
    segments = librosa.util.frame(signal, segment_length_in_samples, hop_length_in_samples)
    return segments
...
def filter_segments(
    ...
    filtered_segments = apply(lambda x: x[:, mask] if len(x.shape) == 2 else x[:, :, mask], segments)
...
def preprocess_single_audio_file(
    ...
    print("Extracting loudness with extractor '%s': %s..." % (loudness_extractor.__name__, file))
    loudness = loudness_extractor(audio)

    print("Extracting MFCC with extractor '%s': %s..." % (mfcc_extractor.__name__, file))
    mfcc = mfcc_extractor(audio)

    print("Segmenting audio file: %s..." % file)
    segmented_audio = segment_signal(audio, target_sr, segment_length_in_seconds, hop_length_in_seconds)
    ...
    (filtered_audio, filtered_f0, filtered_confidence, filtered_loudness, filtered_mfcc,) = filter_segments(
...
def preprocess_audio(
    ...
    if normalise_audio:
        ...
        for file in files:
            ...
            normalisation_factor = max_value if max_value > normalisation_factor else normalisation_factor
