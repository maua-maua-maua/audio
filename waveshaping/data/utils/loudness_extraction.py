...
def compute_power_spectrogram(
    ...
    power_spectrogram = librosa.amplitude_to_db(magnitude_spectrogram, ref=np.max, amin=epsilon)
    return power_spectrogram


def perform_perceptual_weighting(power_spectrogram_in_db: np.ndarray, sample_rate: float, n_fft: int):
...
def extract_perceptual_loudness(
    ...
    if interpolate_fn:
        loudness = interpolate_fn(loudness, n_fft, hop_length, original_length=audio.size)
