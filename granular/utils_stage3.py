...
def export_hierarchical_audio_reconstructions(model, batch, trainset=True):
    ...
    with torch.no_grad():
        ...
        for i in range(audio_output.shape[0]):
            if trainset:
                sf.write(
                    os.path.join(model.export_dir, "audio_train_reconstruction_" + str(i) + ".wav"),
                    audio_export[i, :],
                    model.w_model.hparams.sr,
                )
            else:
                sf.write(
                    os.path.join(model.export_dir, "audio_test_reconstruction_" + str(i) + ".wav"),
                    audio_export[i, :],
                    model.w_model.hparams.sr,
                )


def export_random_samples(model, export_dir, n_samples=10, temperature=1.0):
