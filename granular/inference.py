...
def generate_loop(model, loop_len, drum_dict, temperature=1.0):
    loop_dict = dict()
    for cl in drum_dict:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interp_len", default=4.0, type=float)
    ...
    if args.finetuned:
        ...
        model = hierarchical_model.load_from_checkpoint(
    ...
    sf.write(
        os.path.join(args.export_dir, args.mname + "__sample" + str(args.samples_id) + "_interpolation.wav"),
        audio_interp,
    ...
    sf.write(
        os.path.join(args.export_dir, args.mname + "__sample" + str(args.samples_id) + "_loop.wav"),
        output_loop,
        model.w_model.hparams.sr,
    )
    for cl in drum_dict:
        sf.write(
            os.path.join(args.export_dir, args.mname + "__sample" + str(args.samples_id) + "_loop_" + cl + ".wav"),
            loop_dict[cl],
            model.w_model.hparams.sr,
        )
