...
def make_latent_dataloaders(train_latents, train_labels, test_latents, test_labels, batch_size, num_workers=4):
    ...
    test_dataloader = DataLoader(
        test_dataset, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=num_workers
    )
...
def export_embedding_to_audio_reconstructions(l_model, w_model, batch, trainset=True):
    ...
    with torch.no_grad():
        ...
        for i in range(audio_hat.shape[0]):
            if trainset:
                sf.write(
                    os.path.join(l_model.export_dir, "embedding_to_audio_train_reconstruction_" + str(i) + ".wav"),
                    audio_export[i, :],
                    w_model.hparams.sr,
                )
            else:
                sf.write(
                    os.path.join(l_model.export_dir, "embedding_to_audio_test_reconstruction_" + str(i) + ".wav"),
                    audio_export[i, :],
                    w_model.hparams.sr,
                )


def export_random_samples(l_model, w_model, export_dir, n_samples=10, temperature=1.0):
