...
class waveform_model(pl.LightningModule):
    def __init__(
        ...
        encoder_convs = [
            nn.Sequential(stride_conv(kernel_size, 1, channels, stride), residual_conv(channels, n_blocks=3))
        ]
        encoder_convs += [
            nn.Sequential(stride_conv(kernel_size, channels, channels, stride), residual_conv(channels, n_blocks=3))
    ...
    def init_spectral_distances(
        ...
        self.spec_dist = spectral_distances(
...
class hierarchical_model(pl.LightningModule):
    def __init__(
        ...
        self.w_model = waveform_model.load_from_checkpoint(
            checkpoint_path=w_ckpt_file, hparams_file=w_yaml_file, map_location="cpu"
        )
        self.l_model = latent_model.load_from_checkpoint(
